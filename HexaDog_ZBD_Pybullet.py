...
LEG_ROOTS = {
    "FL": [0.12, 0.10], "ML": [0.00, 0.10], "RL": [-0.12, 0.10],
    "FR": [0.12, -0.10], "MR": [0.00, -0.10], "RR": [-0.12, -0.10]
}
...
try:
    while True:
        ...
        if joystick:
            ...
            # Default values when no joystick is connected
            joy_fwd = 0.0
            joy_side = 0.0
            joy_spin = 0.0
            norm_mag = 0.0
        ...
        # KEYBOARD CONTROL
        #
        # Keyboard is checked independently of the joystick.
        # Therefore keyboard works even when the F710 is connected.
        #
        # UP    = Forward
        # DOWN  = Backward
        # LEFT  = Strafe Left
        ...
        # --------------------------------------------------------
        # KEYBOARD OVERRIDES GAMEPAD MOVEMENT WHEN PRESSED
        # --------------------------------------------------------
        if abs(keyboard_fwd) > 0.0:
            joy_fwd = keyboard_fwd

        if abs(keyboard_side) > 0.0:
            joy_side = keyboard_side

        if abs(keyboard_spin) > 0.0:
            joy_spin = keyboard_spin

        joy_mag = math.sqrt(joy_fwd**2 + joy_side**2)
        norm_mag = min(1.0, joy_mag)

        # --------------------------------------------------------
        # REMAINING CONTROL / GAIT LOGIC
        # --------------------------------------------------------
        ...
        if phase_accumulator >= 1.0:
            ...
            if cycle_time_elapsed > 0:
                avg_speed_cms = (
                    distance / cycle_time_elapsed
                ) * 100.0

                cycle_count += 1

                print(
                    f"[Cycle {cycle_count:03d}] "
                    f"Speed: {avg_speed_cms:6.2f} cm/s | "
                    f"Freq: {g['freq']:.2f}Hz | "
                    f"Target Body H: {current_body_height:.3f}m"
                )
        ...
        for idx, leg in enumerate(LEG_ORDER):
            ...
            phase = (
                phase_accumulator + LEG_PHASES[idx]
            ) % 1.0

            s_phase = (
                phase if phase < 0.5 else phase - 0.5
            ) * 2.0

            cycloid_factor = (
                s_phase -
                (
                    math.sin(2.0 * math.pi * s_phase) /
                    (2.0 * math.pi)
                )
            )
